...
class RL:
    def __init__(self, env, num_states=None, num_actions=None, state_mapper=None):
    ...
    def get_policy(self, Q=None, epsilon_soft=True, epsilon=0.1):
        ...
        if (epsilon_soft):
            policy = [
                [1/self.num_actions for j in range(self.num_actions)] for i in range(self.num_states)]
            for state in range(self.num_states):
                ...
                for action in range(self.num_actions):
                    if Q[state][action] == max:
                        policy[state][action] = (
                            1 - (self.num_actions - len(max_index)) * epsilon / self.num_actions) / len(max_index)
                    else:
            ...
            for state in range(self.num_states):
                ...
                max_index = np.where(q == max)[0]
                policy[state] = [
                    1/len(max_index) if i in max_index else 0 for i in range(self.num_actions)]
    ...
    def on_policy_mc(self, episodes=1000, first_visit=True, gamma=0.9, epsilon=0.1):
        ...
        returns = [[{'sum': 0, 'n': 0} for j in range(
            self.num_actions)] for i in range(self.num_states)]
        ...
        for e in range(episodes):
            ...
            for t in range(len(states) - 1, -1, -1):
                ...
                if (not first_visit or visited[key] == t):
                    ...
                    Q[state, action] = returns[state][action]['sum'] / \
                        returns[state][action]['n']
                    policy = self.get_policy(Q, True, epsilon)
        return Q
    ...
    def off_policy_mc(self, episodes=1000, gamma=0.9, epsilon=0.1):
        ...
        Q = np.zeros((self.num_states, self.num_actions))
        C = [[0 for j in range(self.num_actions)]
